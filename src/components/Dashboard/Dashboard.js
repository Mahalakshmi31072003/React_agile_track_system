...
const Dashboard = () => {
    ...
    const handleAddScrum = async (event) => {
        ...
        try {
            ...
            // Log the new task response for debugging or future use
            console.log('New task added:', newTaskResponse.data);
    
            // Update state with the new scrum including the new task
            setScrums((prevScrums) => [...prevScrums, newScrum]);
    
            // Hide the form and reset input fields after successful submission
            setShowForm(false);
            setNewScrumName('');
            setNewTaskTitle('');
